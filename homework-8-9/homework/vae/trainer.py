...
class Trainer:
    ...
    def __init__(self, model, train_loader, test_loader, optimizer,
                 loss_function, channels, image_size, device='cpu'):
        self.model = model
    ...
    def train(self, n_epoch, log_interval):
        for epoch in range(n_epoch):
            self.__train(epoch + 1, log_interval)
            self.test(epoch + 1, log_interval)
            self.save(epoch)

    def __train(self, epoch, log_interval):
        self.model.train()
        epoch_loss = 0
    ...
    def test(self, epoch, log_interval):
        ...
        for batch_idx, (data, _) in enumerate(self.test_loader):
            ...
            if batch_idx % log_interval == 0:
                ...
                self.plot_generated(data, recon_x)
    ...
    def plot_generated(self, data, recon_x):
        x = vutils.make_grid(data[:5, :, :, :], normalize=True, scale_each=True)
        self.writer.add_image('img/real', x)

        x = vutils.make_grid(recon_x.reshape(-1, self.channels, self.image_size, self.image_size)[:5, :, :, :], normalize=True, scale_each=True)
        self.writer.add_image('img/reconstructed', x)

    def save(self, epoch):
        checkpoint_path = os.path.join('ckpt', f'epoch_{epoch}.pt')
